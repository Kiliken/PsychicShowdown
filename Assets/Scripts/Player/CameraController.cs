...
public class CameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetAxisRaw("Cam1X") != 0 && Input.GetAxisRaw("Cam1Y") != 0){
            rotationX += Input.GetAxis("Cam1Y") * invertYVal * RotSpeedX * 1.6f;
            rotationX = Mathf.Clamp(rotationX, camVertAngleMin, camVertAngleMax); // clamp vertical angle to not move all the way
            rotationY += Input.GetAxis("Cam1X") * invertXVal *  RotSpeedY * 1.6f;
        }
        // normal input
        else{
        ...
        Vector3 focusPosition = followTarget.position + new Vector3(offsetX, offsetY);
